...
sero = readRDS("ww_sero_for_ssmod_J18.Rds")
...
setwd(paste0("/data/state-deconvolved-case-data/", state)) # Where to find the below .rds & the results and plots are to be saved for the state. Change as needed.
const_interp_obs_frac_df <- read_rds("reinfect-final-thetas-df.rds")
const_interp_obs_frac_df <- const_interp_obs_frac_df %>% 
  group_by(time_value) %>%
  summarise(sum_reinfect = sum(reinfect), sum_cases = sum(cases)) %>% # sum over all variants
  mutate(frac_new_infect = 1 - (sum_reinfect / sum_cases)) %>% # to get frac_new_infect
  tidyr::complete(time_value = seq.Date(start_date, end_date, by = "day")) %>%
  fill(sum_reinfect, sum_cases, frac_new_infect, .direction = "up")

frac_new <- const_interp_obs_frac_df %>% filter(time_value %in% yd$rdd) %>% pull(frac_new_infect)
...
ktrend <- k # allowed to be <= k if want lower-smoothness extrapolation
RQR <- matrix(0, k, k)
...
# Get fixed epsilon and svar from our sample of 7 states.
eps_init = 0.99
svar_init = 3e-06
mult_init = 100
states_samp <- c("CA", "WA", "NY", "NV", "NC", "MN", "HI")
...
# Then use those fixed parameters for the sample of 7 states. Just pass those into the model builder.
fixed = logical(length(1:4))
fixed[1] = TRUE # fix eps
fixed[3] = TRUE # fix svar


inv_ratios_all_states <- lapply(states_samp, function(x) state_model_builder(x, fixed_eps, fixed_svar, mult_init, k = 3L, fixed = fixed) )
names(inv_ratios_all_states) <- states_samp

# Save inv_ratios_all_states as RDS
saveRDS(inv_ratios_all_states, "ww_inv_ratios_all_states_M20.RDS")
