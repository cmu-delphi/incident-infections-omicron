...
sr_cutoff_date = as.Date("2022-02-16")  # This date was dictated based on the minimum end date for the state reinfection data 
# (as there is no such reinfection data provided for the state of Minnesota beyond this time)
...
# Unadjusted infections if string of 0s very long (more than 7 days)
# then 7dav doesn't work very well on infections

# Consider centered 7 day rolling average for each unadj_infect
unadj_window = 7
...
saveRDS(coef_mat, "delta_omicron_coef_mat.RDS")
...
saveRDS(unadj_at_df_list, "unadj_at_df_list_Mar20.RDS")
...
# then grouping the data by time_value and geo_value, and finally computing the 95th and 5th percentiles (across all infection estimates for a day) within each group. 
...
#############################################################################################################################################
# Now, load pop and case data to produce plots & conduct some numerical analysis
# Same Initial settings as Adj_Infections_Sept29.R except for start_date, end_date
...
saveRDS(quantiles_unadj_at_df, file = "quantiles_ww_adj_df.RDS")
...
# Plot specifications
cases_to_plot_df = filtered_df_cases # Can use ww_adj_df if want all cases and not to omit multiple consecutive values
...
# 1. Combine all the data frames in unadj_at_df_list with ids for each of the 100 dfs/trajectories under source
...
ggsave(filename = "fig-infect-cases-omicron-no-dual.pdf", width = 10, height = 7)
...
ggsave(filename = "fig-infect-cases-omicron.pdf", width = 10, height = 7) 
...
saveRDS(df_long, file = "omicron_subvariant_prop.RDS")
...
p_background <- ggplot(df_long %>% mutate(geo_value = State), aes(Date, y = Prop, fill = Variant, group = interaction(geo_value, Variant))) +
  ...
  scale_fill_manual(name = "Variant", values = setNames(variant_colors, unique(df_long$Variant)), breaks = geom_area_breaks) +  # Ensure legend colors are fully opaque
...
p_lines <- ggplot(ww_adj_df) +
  ...
  theme(panel.background = element_rect(fill = "transparent", color = NA),  # transparent background
        ...
        legend.position = "top",
        legend.box.margin = margin(5, 5, 5, 5),  # Inside the legend box
        legend.title = element_text(size = 10.5),
        legend.text = element_text(size = 9.5),
        strip.text = element_text(size = 7, face = "bold", margin = margin(1, 0, 1, 0, "mm")))
...
pdf("fig-infect-cases-omicron-with-shading.pdf", width = 10, height = 7)  
...
ggplot(combined_unadj_at_df_wsource) +
  ...
  theme(axis.text.x = element_text(size = 9.5),
        ...
        legend.position = "top",
        strip.text = element_text(size = 7, face = "bold", margin = margin(1, 0, 1, 0, "mm")))
ggsave(filename = "inverse-reporting-ratios.pdf", width = 10, height = 7)
...
state_to_plot_pop = pop_df %>%
  filter(geo_value == state_to_plot) %>%
  pull(pop_used)

# State WW
ggplot(ww_ts_long_final %>% filter(geo_value == state_to_plot & time_value >= cutoff_date), aes(time_value, y = ww)) +
  geom_point(color = "navy", alpha = 0.7) +
  scale_x_date(name = "", date_breaks = "3 month", date_labels = "%b %Y", expand = c(0.01, 0)) +
  ylab("Average effective genome copies per mL") +
  theme_bw()
ggsave(filename = paste0("ww_state_", state_to_plot, ".pdf"), width = 12, height = 7)


# Deconvolved cases plotted for one state:
unadj_df_statepl <- unadj_df %>% filter(geo_value == state_to_plot & time_value >= cutoff_date) %>% mutate(unadj_infect_rate = (unadj_infect / state_to_plot_pop) * 100000)

ggplot() +
  geom_line(data = unadj_df_statepl, aes(time_value, unadj_infect_rate), color = "darkgreen", size = 1.2) +
  scale_x_date(name = "", date_breaks = "3 month", date_labels = "%b %Y", expand = c(0, 0)) +
  ylab("Deconvolved cases per 100k") +
  theme_bw(16) +
  theme(axis.text.x = element_text(hjust = 1, size = 9.5),
        axis.text.y = element_text(size = 9.5),
        axis.title.y = element_text(size = 11.5),
        strip.text = element_text(size = 7, face = "bold", margin = margin(1, 0, 1, 0, "mm")))
ggsave(filename = paste0("ww_deconvolved_cases_", state_to_plot, ".pdf"), width = 12, height = 7)


# Proportions of variants in circulation plot for the state
...
unadj_prop_circ_df_state <- vmix %>%
  filter(geo_value == state_to_plot) %>%
  filter(time_value >= "2020-06-01" & time_value <= end_date) %>% 
  select(-c(geo_value))

plotter(unadj_prop_circ_df_state %>% select(-time_value), dates = seq(min(unadj_prop_circ_df_state$time_value), max(unadj_prop_circ_df_state$time_value), by = "1 day")) # this is a bit slow
ggsave(filename = paste0("ww_var_props_", state_to_plot, ".pdf"), width = 12, height = 7)

# IRRs for all trajectories
# Filter data frame for state of interest
combined_unadj_at_df_wsource_statepl <- combined_unadj_at_df_wsource %>% filter(geo_value == state_to_plot)
ww_adj_df_statepl <- ww_adj_df %>% filter(geo_value == state_to_plot)

# Plot inverse reporting ratios for all infection trajectories
ggplot() +
  geom_line(data = combined_unadj_at_df_wsource_statepl, aes(time_value, at_val, group = source), color = "maroon", size = 0.2, alpha = 0.5) +
  scale_x_date(name = "", date_breaks = "3 month", date_labels = "%b %Y", expand = c(0, 0)) +
  ylim(c(2, 45)) + 
  ylab("Inverse reporting ratio") +
  theme_bw(16) +
  theme(axis.text.x = element_text(hjust = 1, size = 9.5),
        ...
        axis.title.y = element_text(size = 11.5),
        strip.text = element_text(size = 7, face = "bold", margin = margin(1, 0, 1, 0, "mm")))
ggsave(filename = paste0("ww_inverse_reporting_ratios_", state_to_plot, ".pdf"), width = 12, height = 7)


# Plot all infection trajectories
ggplot(ww_adj_df_statepl) +
  geom_line(data = combined_unadj_at_df_wsource_statepl, aes(time_value, adj_inf_rate, group = source, color = "Inf. trajectories"), size = 0.2, alpha = 0.7) +
  # Overlay median
  geom_line(aes(time_value, adj_inf_q50_rate, color = "Adj. inf. / 100k"), size = 0.65) + 
  scale_x_date(name = "", date_breaks = "3 month", date_labels = "%b %Y", expand = c(0, 0)) +
  scale_y_continuous(labels = scales::comma) +
  scale_color_manual(name = 'Estimates', 
                     breaks = c('Adj. inf. / 100k', 'Inf. trajectories'),
                     values = c('Adj. inf. / 100k' = "midnightblue", 
                                'Inf. trajectories' = "#78C1F2")) +
  theme_bw(16) +
  ...
  theme(axis.text.x = element_text(size = 9.5),
        ...
        legend.box.spacing = unit(-15, "pt"),
        strip.text = element_text(size = 7, face = "bold", margin = margin(1, 0, 1, 0, "mm")))
ggsave(filename = paste0("ww_infection_rates_", state_to_plot, ".pdf"), width = 12, height = 7)


# Plot all infection trajectories and unadjusted infections
ggplot(ww_adj_df_statepl) +
  geom_line(data = combined_unadj_at_df_wsource_statepl, aes(time_value, adj_inf_rate, group = source, color = "Inf. trajectories"), size = 0.2, alpha = 0.7) +
  # Overlay median
  geom_line(aes(time_value, adj_inf_q50_rate, color = "Adj. inf. / 100k"), size = 0.65) + 
  # Overlay uadjusted infections
  geom_line(data = unadj_df_statepl, aes(time_value, unadj_infect_rate, color = "Unadj. inf. / 100k"), size = 0.8) + 
  scale_x_date(name = "", date_breaks = "3 month", date_labels = "%b %Y", expand = c(0, 0)) +
  scale_y_continuous(labels = scales::comma) + #, limits = c(0, 30)) +
  scale_color_manual(name = 'Estimates', 
                     breaks = c('Adj. inf. / 100k', "Unadj. inf. / 100k", 'Inf. trajectories'),
                     values = c('Adj. inf. / 100k' = "midnightblue", 
                                "Unadj. inf. / 100k" = "darkgreen", 
                                'Inf. trajectories' = "#78C1F2")) +
  theme_bw(16) +
  ylab("Incidence per 100k") +
  theme(axis.text.x = element_text(size = 9.5),
        ...
        legend.box.spacing = unit(-15, "pt"),
        strip.text = element_text(size = 7, face = "bold", margin = margin(1, 0, 1, 0, "mm")))
ggsave(filename = paste0("ww_infection_unadj_rates_", state_to_plot, ".pdf"), width = 12, height = 7)
