...
library(lubridate)
library(Matrix) # cv
library(dspline) # d_mat
library(reticulate) # load numpy file
library(covidcast) # covidcast_signal
...
comm_sero2_sub = comm_sero2_sub %>% filter(Rate != 0) # Only one row with Rate = 0 SD on 2020-08-19
...
# Slice last sero value off for each state to see how this impacts final estimates
...
# Manually remove Rate of 0 (else will give infinite weights)
blood_cdc_df = blood_cdc_df %>% filter(Rate != 0) # Only one row with Rate = 0 IA on 2020-07-15
...
# Substitue NA weights for 1s
sero = sero%>% mutate(weights = ifelse(is.na(weights), 1, weights))
# View(sero %>% filter(res_state == "AK"))
...
# Check each state has two rows (one for comm. and one for bd)
#View(sero %>% filter(!is.na(len_obs)) %>% group_by(res_state, Source) %>% slice(1) %>% group_by(res_state) %>% summarise(lenres = length(res_state)))
